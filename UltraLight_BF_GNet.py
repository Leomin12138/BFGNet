...
from BFEM import Bimodal_Foreground_Enhancement_Module as BFEM


class FCSS(nn.Module):
...
class UltraLight_BF_UNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=4, c_list=[32, 64, 128, 128, 128, 128],
        ...
        self.encoder4 = nn.Sequential(
            FCSS(input_dim=c_list[2], output_dim=c_list[3])
        )
        self.encoder5 = nn.Sequential(
            FCSS(input_dim=c_list[3], output_dim=c_list[4])
        )
        self.encoder6 = nn.Sequential(
            FCSS(input_dim=c_list[4], output_dim=c_list[5])
        )

        # Decoder
        self.decoder1 = nn.Sequential(
            FCSS(input_dim=c_list[5], output_dim=c_list[4])
        )
        self.decoder2 = nn.Sequential(
            FCSS(input_dim=c_list[4], output_dim=c_list[3])
        )
        self.decoder3 = nn.Sequential(
            FCSS(input_dim=c_list[3], output_dim=c_list[2])
        )
        ...
        self.FFM_1 = BFEM(in_channels=32)
        self.FFM_2 = BFEM(in_channels=64)
        self.FFM_3 = BFEM(in_channels=128)
