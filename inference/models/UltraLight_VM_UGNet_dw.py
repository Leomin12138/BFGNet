...
class UltraLight_VM_UNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=4, c_list=[32, 64, 128, 128, 128, 128], bridge=True):
        super().__init__()
        self.bridge = bridge

        self.encoder1_rgb = nn.Sequential(
            nn.Conv2d(input_channels - 1, c_list[0], 3, stride=2, padding=1),
            nn.Conv2d(c_list[0], c_list[0], 1)
        )

        self.encoder1_depth = nn.Sequential(
            nn.Conv2d(input_channels - 3, c_list[0], 3, stride=2, padding=1),
            nn.Conv2d(c_list[0], c_list[0], 1)
        )

        self.encoder2_rgb = nn.Sequential(
            nn.Conv2d(c_list[0], c_list[0], 3, stride=2, padding=1),
        ...
        self.encoder3_rgb = nn.Sequential(
            nn.Conv2d(c_list[1], c_list[1], 3, stride=2, padding=1),
    ...
    def forward(self, x):
        ...
        x6 = F.gelu(self.encoder6(x5))

        out5 = F.gelu(self.dbn1(self.decoder1(x6)))
        out5 = torch.add(out5, x5)
        out5 = self.conv_skip5(torch.cat([out5, x5], dim=1))

        out4 = F.gelu(F.interpolate(self.dbn2(self.decoder2(out5)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))
        out4 = torch.add(out4, x4)
        out4 = self.conv_skip4(torch.cat([out4, x4], dim=1))

        out3 = F.gelu(F.interpolate(self.dbn3(self.decoder3(out4)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))
        out3 = torch.add(out3, x3)
        out3 = self.conv_skip3(torch.cat([out3, x3], dim=1))

        out2 = F.gelu(F.interpolate(self.dbn4(self.decoder4(out3)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))
        out2 = torch.add(out2, x2)
        out2 = self.conv_skip2(torch.cat([out2, x2], dim=1))

        out1 = F.gelu(F.interpolate(self.dbn5(self.decoder5(out2)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))
        out1 = torch.add(out1, x1)
        out1 = self.conv_skip1(torch.cat([out1, x1], dim=1))
