...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        # print(f"selective_scan_backend: {selective_scan_backend}")  # 打印当前值
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                # print("self.drop_path(self.op(self.norm(x))) shape:", self.drop_path(self.op(self.norm(x))).shape)
...
class PVMLayer(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.mamba = VSSBlock(
            hidden_dim=input_dim//4,  # 必须指定
            drop_path=0.2,  # 可选参数，使用默认值 0
            norm_layer=nn.LayerNorm,  # 可选参数，使用默认值 nn.LayerNorm
            channel_first=False,  # 可选参数，使用默认值 False
            ssm_d_state=16,  # 可选参数，使用默认值 16
            ssm_ratio=2.0,  # 可选参数，使用默认值 2.0
            ssm_dt_rank="auto",  # 可选参数，使用默认值 "auto"
            ssm_act_layer=nn.SiLU,  # 可选参数，使用默认值 nn.SiLU
            ssm_conv=3,  # 可选参数，使用默认值 3
            ssm_conv_bias=True,  # 可选参数，使用默认值 True
            ssm_drop_rate=0.0,  # 可选参数，使用默认值 0.0
            ssm_init="v0",  # 可选参数，使用默认值 "v0"
            forward_type="v05_noz",  # 可选参数，使用默认值 "v2"
            mlp_ratio=4.0,  # 可选参数，使用默认值 4.0
            mlp_act_layer=nn.GELU,  # 可选参数，使用默认值 nn.GELU
            mlp_drop_rate=0.0,  # 可选参数，使用默认值 0.0
            gmlp=False,  # 可选参数，使用默认值 False
            use_checkpoint=False,  # 可选参数，使用默认值 False
            post_norm=False,  # 可选参数，使用默认值 False
        )
    ...
    def forward(self, x):
        ...
        x1, x2, x3, x4 = torch.chunk(x, 4, dim=3)
...
if __name__ == "__main__":
    ...
    model = PVMLayer(input_dim=8,output_dim=8)
